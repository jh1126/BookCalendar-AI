...
template_embeddings = sbert_model.encode(template_texts_clean, convert_to_tensor=True)
...
def find_similar_templates_for_keyword(keyword):
    ...
    kw_emb = sbert_model.encode(input_text, convert_to_tensor=True)
    cos_scores = util.cos_sim(kw_emb, template_embeddings).squeeze()
    scored_templates = [(template_data["questions"][i]["template"], score.item())
                        for i, score in enumerate(cos_scores) if score.item() >= 0.4]
