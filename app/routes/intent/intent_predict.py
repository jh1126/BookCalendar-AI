...
def generate_recommendation(text: str):
    prompt = (
        ...
        f"\"{text}\"\n\n"
        f"이 요청을 참고하여 요청이 일상대화라면 그에 맞는 답변을 해주고, 그게 아닌 추천을 원한다면 사용자가 원하는 도서, 장르, 카테고리, 분위기, 스토리 등을 고려한 책을 세 권 추천해주고 추천이유도 설명해주세요."
    )
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
    ...
    if intent == "질문":
        ...
        try:
            with conn.cursor() as cursor:
                sql = "INSERT INTO intentionData (date, input, output) VALUES (%s, %s, %s)"
                cursor.execute(sql, (date_str, text, intent))
            conn.commit()
        finally:
            conn.close()
